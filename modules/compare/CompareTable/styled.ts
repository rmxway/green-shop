...
export const CharacteristicLabel = styled.td<{ $index?: number }>`
	${({ theme, $index }) => css`
		...
		min-width: 150px;
		max-width: 250px;
		background-color: ${$index !== undefined && $index % 2 === 0
			? theme.colors.gray.$1
			: darken(0.01, theme.colors.gray.$1)};
		z-index: 1;
		border-right: 1px solid ${theme.colors.gray.$2};
		box-shadow: 2px 0 4px rgba(0, 0, 0, 0.05);
		transition: all 0.1s;
...
export const CharacteristicCell = styled(motion.td)<{ $highlight?: boolean; $index?: number }>`
	${({ theme, $highlight, $index }) => css`
		...
		background-color: ${$index !== undefined && $index % 2 === 0
			? theme.colors.gray.$1
			: darken(0.02, theme.colors.gray.$1)};
		...
		&:after {
			...
			left: 0;
			top: 0;
			background-color: ${$highlight ? opacify(-0.85, theme.colors.label) : 'none'};
			opacity: 1;
			z-index: 0;
		}
